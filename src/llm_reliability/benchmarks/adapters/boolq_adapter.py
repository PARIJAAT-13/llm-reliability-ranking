...
Purpose
-------
Integrate the BoolQ benchmark into the framework.

Responsibilities
----------------
- Load BoolQ yes/no reading comprehension tasks
- Evaluate by detecting "true"/"false" or "yes"/"no" in agent responses
"""
...
class BoolQAdapter(BaseBenchmarkAdapter):
    """Adapter for the BoolQ benchmark."""

    def validate_configuration(self) -> None:
        super().validate_configuration()
        if not self.config.metadata.get("dataset_path"):
            raise ValueError("Configuration metadata must contain 'dataset_path' for BoolQ.")
    ...
    def _load_tasks(self) -> None:
        ...
        if path_obj.exists() and path_obj.is_file():
            ...
            if not isinstance(data, list):
                raise TypeError("BoolQ dataset must be a list of tasks.")

            self._tasks = {}
            for item in data:
                tid = item.get("task_id", f"boolq_{len(self._tasks)}")
                passage = item.get("passage", "")
                question = item.get("question", "")
                answer = str(item.get("answer", item.get("label", "true"))).strip().upper()
            ...
            self._tasks = {
                f"boolq_{i}": {
                    ...
                    "question": f"Does the Earth orbit the Sun? {i}",
                    "prompt": f"Passage: The Earth orbits the Sun.\nQuestion: Does the Earth orbit the Sun? {i}\nAnswer (True/False):",
    ...
    def run(self, agent: Agent, task: dict[str, Any]) -> ExecutionRecord:
        ...
        record = ExecutionRecord(
            ...
            timestamp=timestamp,
            stdout="BoolQ Execution",
            stderr="",
            status=status,
            error=error,
            agent_output=agent_output,
            software_versions={"boolq": "1.0"},
    ...
    def evaluate(self, execution: ExecutionRecord) -> EvaluationRecord:
        ...
        expected = task.get("ground_truth_answer", "TRUE").upper()
        ...
        if execution.status == "error" or not agent_output:
            ...
            extracted = extract_boolq_answer(str(agent_output))
...
BenchmarkRegistry.register("BoolQ", BoolQAdapter)
