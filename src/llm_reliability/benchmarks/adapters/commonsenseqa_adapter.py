...
Purpose
-------
Integrate the CommonsenseQA benchmark into the framework.

Responsibilities
----------------
- Load CommonsenseQA multiple-choice commonsense reasoning tasks
- Evaluate answer extraction from agent responses (A-E)
"""
...
def extract_commonsenseqa_choice(text: str) -> str:
    if not text:
        return ""
    match = re.search(r"\b([A-E])\b", text.strip(), re.IGNORECASE)
    if match:
        return match.group(1).upper()
    return text.strip()[:1].upper()


class CommonsenseQAAdapter(BaseBenchmarkAdapter):
    """Adapter for the CommonsenseQA benchmark."""

    def validate_configuration(self) -> None:
        super().validate_configuration()
        if not self.config.metadata.get("dataset_path"):
            raise ValueError(
                "Configuration metadata must contain 'dataset_path' for CommonsenseQA."
            )
    ...
    def _load_tasks(self) -> None:
        ...
        if path_obj.exists() and path_obj.is_file():
            ...
            if not isinstance(data, list):
                raise TypeError("CommonsenseQA dataset must be a list of tasks.")

            self._tasks = {}
            for item in data:
                tid = item.get("task_id", f"commonsenseqa_{len(self._tasks)}")
                question = item.get("question", item.get("prompt", ""))
                choices = item.get("choices", item.get("options", []))
                answer = (
                    str(item.get("answer", item.get("ground_truth_answer", "A"))).strip().upper()
                )
            ...
            self._tasks = {
                f"commonsenseqa_{i}": {
                    "task_id": f"commonsenseqa_{i}",
                    "question": f"Where do you keep your shoes? {i}",
                    "choices": [
                        "On a bookshelf",
                        "In a closet",
                        "In the fridge",
                    ...
                    "prompt": f"Where do you keep your shoes? {i}\nA. On a bookshelf\nB. In a closet\nC. In the fridge\nD. Under the bed\nE. On the roof\nAnswer:",
    ...
    def run(self, agent: Agent, task: dict[str, Any]) -> ExecutionRecord:
        ...
        record = ExecutionRecord(
            ...
            timestamp=timestamp,
            stdout="CommonsenseQA Execution",
            stderr="",
            status=status,
            error=error,
            agent_output=agent_output,
            software_versions={"commonsenseqa": "1.0"},
    ...
    def evaluate(self, execution: ExecutionRecord) -> EvaluationRecord:
        ...
        if execution.status == "error" or not agent_output:
            ...
            extracted = extract_commonsenseqa_choice(str(agent_output))
...
BenchmarkRegistry.register("CommonsenseQA", CommonsenseQAAdapter)
