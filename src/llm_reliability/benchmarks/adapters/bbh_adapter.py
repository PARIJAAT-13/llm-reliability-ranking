...
Purpose
-------
Integrate the BIG-Bench Hard (BBH) benchmark into the framework.

Responsibilities
----------------
- Load BBH reasoning tasks with question, choices, and target answer
- Evaluate multiple-choice extraction from agent responses
"""
...
def extract_bbh_choice(text: str) -> str:
    if not text:
        return ""
...
class BBHAdapter(BaseBenchmarkAdapter):
    """Adapter for the BIG-Bench Hard benchmark."""

    def validate_configuration(self) -> None:
        super().validate_configuration()
        if not self.config.metadata.get("dataset_path"):
            raise ValueError("Configuration metadata must contain 'dataset_path' for BBH.")
    ...
    def _load_tasks(self) -> None:
        ...
        if path_obj.exists() and path_obj.is_file():
            ...
            if not isinstance(data, list):
                raise TypeError("BBH dataset must be a list of tasks.")

            self._tasks = {}
            for item in data:
                tid = item.get("task_id", item.get("id", f"bbh_{len(self._tasks)}"))
                question = item.get("question", item.get("input", ""))
                choices = item.get("choices", item.get("options", []))
                answer = str(item.get("answer", item.get("target", "A"))).strip().upper()
            ...
            self._tasks = {
                f"bbh_{i}": {
                    "task_id": f"bbh_{i}",
                    "question": f"Which number is the largest? {i}",
                    "choices": ["1", "10", "100", "1000"],
                    "prompt": f"Which number is the largest? {i}\nA. 1\nB. 10\nC. 100\nD. 1000\nAnswer:",
    ...
    def run(self, agent: Agent, task: dict[str, Any]) -> ExecutionRecord:
        ...
        record = ExecutionRecord(
            ...
            timestamp=timestamp,
            stdout="BBH Execution",
            stderr="",
            status=status,
            error=error,
            agent_output=agent_output,
            software_versions={"bbh": "1.0"},
    ...
    def evaluate(self, execution: ExecutionRecord) -> EvaluationRecord:
        ...
        if execution.status == "error" or not agent_output:
            ...
            extracted = extract_bbh_choice(str(agent_output))
...
BenchmarkRegistry.register("BBH", BBHAdapter)
