...
class _GroqAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'groq' package is required for GroqAgent.") from exc
        api_key = os.environ.get("GROQ_API_KEY")
        if not api_key:
            raise AuthenticationError("GROQ_API_KEY environment variable is not set.")
        self._client = groq.Groq(api_key=api_key)
        logger.info("Groq client initialised (model=%s).", self._model)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_GroqAdapter.generate() called before initialize().")
        try:
            import groq
        except ImportError as exc:
            raise ProviderError("groq package not available.") from exc
        ...
        try:
            ...
            raise AuthenticationError(f"Groq auth failed: {exc}") from exc
        except groq.RateLimitError as exc:
            raise RateLimitError(f"Groq rate limit: {exc}") from exc
        except groq.APIConnectionError as exc:
            raise ProviderConnectionError(f"Groq network error: {exc}") from exc
        ...
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("Groq returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "groq", "model": self._model}
...
class GroqAgent(BaseProvider):
    ...
    def __init__(self, config: Configuration) -> None:
        super().__init__(config)
        self._adapter = _GroqAdapter(config)
    ...
    def metadata(self) -> dict[str, Any]:
        base = super().metadata()
        base.update({"name": "GroqAgent", "provider": "groq", "model": self._adapter._model})
        return base
...
if not ProviderRegistry.exists("groq"):
    ProviderRegistry.register("groq", _GroqAdapter)
if not RuntimeRegistry.exists("groq"):
    RuntimeRegistry.register("groq", GroqAgent)
