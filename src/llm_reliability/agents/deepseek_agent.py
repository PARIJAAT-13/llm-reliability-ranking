"""
DeepSeekAgent — DeepSeek provider adapter for LLM Reliability Ranking.

DeepSeek exposes an OpenAI-compatible Chat Completions API, so this adapter
reuses the OpenAI SDK with ``base_url`` pointed at DeepSeek's endpoint.

    DeepSeekAgent(Agent)
      └── _DeepSeekAdapter(BaseLLMAdapter)
...
class _DeepSeekAdapter(BaseLLMAdapter):
    """Internal DeepSeek adapter (OpenAI-compatible API)."""
    ...
    def __init__(self, config: Configuration) -> None:
        ...
        if self._model and not self._model.startswith("deepseek"):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError(
                "The 'openai' package is required for DeepSeekAgent (OpenAI-compat API). "
                "Install with: pip install openai>=1.0"
            ) from exc

        api_key = os.environ.get("DEEPSEEK_API_KEY")
        if not api_key:
            raise AuthenticationError("DEEPSEEK_API_KEY environment variable is not set.")
    ...
    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_DeepSeekAdapter.generate() called before initialize().")
...
class DeepSeekAgent(BaseProvider):
    ...
    def shutdown(self) -> None:
        logger.info("Shutting down DeepSeekAgent.")
