...
class _FireworksAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'openai' package is required for Fireworks.") from exc
        api_key = os.environ.get("FIREWORKS_API_KEY")
        if not api_key:
            raise AuthenticationError("FIREWORKS_API_KEY environment variable is not set.")
        self._client = openai.OpenAI(
            base_url="https://api.fireworks.ai/inference/v1",
            api_key=api_key,
        )
        logger.info("Fireworks client initialised (model=%s).", self._model)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_FireworksAdapter.generate() called before initialize().")
        try:
            import openai
        except ImportError as exc:
        ...
        try:
            ...
            raise AuthenticationError(f"Fireworks auth failed: {exc}") from exc
        except openai.RateLimitError as exc:
            raise RateLimitError(f"Fireworks rate limit: {exc}") from exc
        except openai.APIConnectionError as exc:
            raise ProviderConnectionError(f"Fireworks network error: {exc}") from exc
        except openai.APIError as exc:
            raise ProviderError(f"Fireworks API error: {exc}") from exc
        latency_ms = (time.perf_counter() - t0) * 1000.0
        choice = completion.choices[0] if completion.choices else None
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("Fireworks returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "fireworks", "model": self._model}
...
class FireworksAgent(BaseProvider):
    ...
    def __init__(self, config: Configuration) -> None:
        super().__init__(config)
        self._adapter = _FireworksAdapter(config)
    ...
    def metadata(self) -> dict[str, Any]:
        base = super().metadata()
        base.update(
            {"name": "FireworksAgent", "provider": "fireworks", "model": self._adapter._model}
        )
        return base
...
if not ProviderRegistry.exists("fireworks"):
    ProviderRegistry.register("fireworks", _FireworksAdapter)
if not RuntimeRegistry.exists("fireworks"):
    RuntimeRegistry.register("fireworks", FireworksAgent)
