...
class _SambaNovaAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'openai' package is required for SambaNova.") from exc
        api_key = os.environ.get("SAMBANOVA_API_KEY")
        if not api_key:
            raise AuthenticationError("SAMBANOVA_API_KEY environment variable is not set.")
        self._client = openai.OpenAI(
            base_url="https://api.sambanova.ai/v1",
            api_key=api_key,
        )
        logger.info("SambaNova client initialised (model=%s).", self._model)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_SambaNovaAdapter.generate() called before initialize().")
        try:
            import openai
        except ImportError as exc:
        ...
        try:
            ...
            raise AuthenticationError(f"SambaNova auth failed: {exc}") from exc
        except openai.RateLimitError as exc:
            raise RateLimitError(f"SambaNova rate limit: {exc}") from exc
        except openai.APIConnectionError as exc:
            raise ProviderConnectionError(f"SambaNova network error: {exc}") from exc
        except openai.APIError as exc:
            raise ProviderError(f"SambaNova API error: {exc}") from exc
        latency_ms = (time.perf_counter() - t0) * 1000.0
        choice = completion.choices[0] if completion.choices else None
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("SambaNova returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "sambanova", "model": self._model}
...
class SambaNovaAgent(BaseProvider):
    ...
    def metadata(self) -> dict[str, Any]:
        base = super().metadata()
        base.update(
            {"name": "SambaNovaAgent", "provider": "sambanova", "model": self._adapter._model}
        )
        return base
...
if not ProviderRegistry.exists("sambanova"):
    ProviderRegistry.register("sambanova", _SambaNovaAdapter)
if not RuntimeRegistry.exists("sambanova"):
    RuntimeRegistry.register("sambanova", SambaNovaAgent)
