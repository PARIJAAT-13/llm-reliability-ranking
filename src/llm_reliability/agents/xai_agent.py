...
class _XAIAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'openai' package is required for xAI (Grok).") from exc
        api_key = os.environ.get("XAI_API_KEY")
        if not api_key:
            raise AuthenticationError("XAI_API_KEY environment variable is not set.")
        self._client = openai.OpenAI(
            base_url="https://api.x.ai/v1",
            api_key=api_key,
        )
        logger.info("xAI client initialised (model=%s).", self._model)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_XAIAdapter.generate() called before initialize().")
        try:
            import openai
        except ImportError as exc:
        ...
        try:
            ...
            raise AuthenticationError(f"xAI auth failed: {exc}") from exc
        except openai.RateLimitError as exc:
            raise RateLimitError(f"xAI rate limit: {exc}") from exc
        except openai.APIConnectionError as exc:
            raise ProviderConnectionError(f"xAI network error: {exc}") from exc
        ...
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("xAI returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "xai", "model": self._model}
...
class XAIAgent(BaseProvider):
    ...
    def __init__(self, config: Configuration) -> None:
        super().__init__(config)
        self._adapter = _XAIAdapter(config)
    ...
    def metadata(self) -> dict[str, Any]:
        base = super().metadata()
        base.update({"name": "XAIAgent", "provider": "xai", "model": self._adapter._model})
        return base
...
if not ProviderRegistry.exists("xai"):
    ProviderRegistry.register("xai", _XAIAdapter)
if not RuntimeRegistry.exists("xai"):
    RuntimeRegistry.register("xai", XAIAgent)
