...
class _NIMAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'openai' package is required for NVIDIA NIM.") from exc
        ...
        if not api_key:
            raise AuthenticationError(
                "NVIDIA_API_KEY or NGC_API_KEY environment variable must be set."
            )
        self._client = openai.OpenAI(base_url=base_url, api_key=api_key)
        logger.info("NVIDIA NIM client initialised (model=%s).", self._model)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_NIMAdapter.generate() called before initialize().")
        try:
            import openai
        except ImportError as exc:
        ...
        try:
            ...
            raise AuthenticationError(f"NVIDIA NIM auth failed: {exc}") from exc
        except openai.RateLimitError as exc:
            raise RateLimitError(f"NVIDIA NIM rate limit: {exc}") from exc
        except openai.APIConnectionError as exc:
            raise ProviderConnectionError(f"NVIDIA NIM network error: {exc}") from exc
        except openai.APIError as exc:
            raise ProviderError(f"NVIDIA NIM API error: {exc}") from exc
        latency_ms = (time.perf_counter() - t0) * 1000.0
        choice = completion.choices[0] if completion.choices else None
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("NVIDIA NIM returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "nvidia_nim", "model": self._model}
...
class NIMAgent(BaseProvider):
    ...
    def __init__(self, config: Configuration) -> None:
        super().__init__(config)
        self._adapter = _NIMAdapter(config)
...
if not ProviderRegistry.exists("nvidia_nim"):
    ProviderRegistry.register("nvidia_nim", _NIMAdapter)
if not RuntimeRegistry.exists("nvidia_nim"):
    RuntimeRegistry.register("nvidia_nim", NIMAgent)
