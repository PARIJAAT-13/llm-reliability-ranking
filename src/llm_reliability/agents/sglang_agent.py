...
class _SGLangAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'openai' package is required for SGLang.") from exc
        base_url = os.environ.get("SGLANG_BASE_URL", "http://127.0.0.1:30000/v1")
        api_key = os.environ.get("SGLANG_API_KEY", "none")
        self._client = openai.OpenAI(base_url=base_url, api_key=api_key)
        logger.info("SGLang client initialised (model=%s, url=%s).", self._model, base_url)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_SGLangAdapter.generate() called before initialize().")
        try:
            import openai
        except ImportError as exc:
        ...
        try:
            ...
            raise AuthenticationError(f"SGLang auth failed: {exc}") from exc
        except openai.RateLimitError as exc:
            raise RateLimitError(f"SGLang rate limit: {exc}") from exc
        except openai.APIConnectionError as exc:
            raise ProviderConnectionError(f"SGLang network error: {exc}") from exc
        except openai.APIError as exc:
            raise ProviderError(f"SGLang API error: {exc}") from exc
        latency_ms = (time.perf_counter() - t0) * 1000.0
        choice = completion.choices[0] if completion.choices else None
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("SGLang returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "sglang", "model": self._model}
...
class SGLangAgent(BaseProvider):
    ...
    def __init__(self, config: Configuration) -> None:
        super().__init__(config)
        self._adapter = _SGLangAdapter(config)
    ...
    def metadata(self) -> dict[str, Any]:
        base = super().metadata()
        base.update({"name": "SGLangAgent", "provider": "sglang", "model": self._adapter._model})
        return base
...
if not ProviderRegistry.exists("sglang"):
    ProviderRegistry.register("sglang", _SGLangAdapter)
if not RuntimeRegistry.exists("sglang"):
    RuntimeRegistry.register("sglang", SGLangAgent)
