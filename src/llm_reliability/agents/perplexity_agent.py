...
class _PerplexityAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'openai' package is required for Perplexity.") from exc
        api_key = os.environ.get("PERPLEXITY_API_KEY")
        if not api_key:
            raise AuthenticationError("PERPLEXITY_API_KEY environment variable is not set.")
        self._client = openai.OpenAI(
            base_url="https://api.perplexity.ai",
            api_key=api_key,
        )
        logger.info("Perplexity client initialised (model=%s).", self._model)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_PerplexityAdapter.generate() called before initialize().")
        try:
            import openai
        except ImportError as exc:
        ...
        try:
            ...
            raise AuthenticationError(f"Perplexity auth failed: {exc}") from exc
        except openai.RateLimitError as exc:
            raise RateLimitError(f"Perplexity rate limit: {exc}") from exc
        except openai.APIConnectionError as exc:
            raise ProviderConnectionError(f"Perplexity network error: {exc}") from exc
        except openai.APIError as exc:
            raise ProviderError(f"Perplexity API error: {exc}") from exc
        latency_ms = (time.perf_counter() - t0) * 1000.0
        choice = completion.choices[0] if completion.choices else None
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("Perplexity returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "perplexity", "model": self._model}
...
class PerplexityAgent(BaseProvider):
    provider_name: str = "perplexity"
    default_model: str = "sonar-small-chat"
    ...
    def __init__(self, config: Configuration) -> None:
        super().__init__(config)
        self._adapter = _PerplexityAdapter(config)
    ...
    def metadata(self) -> dict[str, Any]:
        base = super().metadata()
        base.update(
            {"name": "PerplexityAgent", "provider": "perplexity", "model": self._adapter._model}
        )
        return base
...
if not ProviderRegistry.exists("perplexity"):
    ProviderRegistry.register("perplexity", _PerplexityAdapter)
if not RuntimeRegistry.exists("perplexity"):
    RuntimeRegistry.register("perplexity", PerplexityAgent)
