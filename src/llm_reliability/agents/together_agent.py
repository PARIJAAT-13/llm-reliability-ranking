...
class _TogetherAdapter(BaseLLMAdapter):
    ...
    def initialize(self) -> None:
        try:
            ...
            raise ImportError("The 'openai' package is required for Together AI.") from exc
        api_key = os.environ.get("TOGETHER_API_KEY")
        if not api_key:
            raise AuthenticationError("TOGETHER_API_KEY environment variable is not set.")
        self._client = openai.OpenAI(
            base_url="https://api.together.xyz/v1",
            api_key=api_key,
        )
        logger.info("Together AI client initialised (model=%s).", self._model)

    def generate(self, request: LLMRequest) -> LLMResponse:
        if self._client is None:
            raise RuntimeError("_TogetherAdapter.generate() called before initialize().")
        try:
            import openai
        except ImportError as exc:
        ...
        try:
            ...
            raise AuthenticationError(f"Together AI auth failed: {exc}") from exc
        except openai.RateLimitError as exc:
            raise RateLimitError(f"Together AI rate limit: {exc}") from exc
        except openai.APIConnectionError as exc:
            raise ProviderConnectionError(f"Together AI network error: {exc}") from exc
        ...
        if choice is None or not getattr(choice.message, "content", None):
            raise ResponseValidationError("Together AI returned empty completion.")
    ...
    def provider_metadata(self) -> dict[str, Any]:
        return {"provider": "together", "model": self._model}
...
class TogetherAgent(BaseProvider):
    ...
    def __init__(self, config: Configuration) -> None:
        super().__init__(config)
        self._adapter = _TogetherAdapter(config)
    ...
    def metadata(self) -> dict[str, Any]:
        base = super().metadata()
        base.update(
            {"name": "TogetherAgent", "provider": "together", "model": self._adapter._model}
        )
        return base
...
if not ProviderRegistry.exists("together"):
    ProviderRegistry.register("together", _TogetherAdapter)
if not RuntimeRegistry.exists("together"):
    RuntimeRegistry.register("together", TogetherAgent)
